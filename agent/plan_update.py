...
class PlanUpdate:
  ...
  def from_dict(
    ...
    if not isinstance(payload, dict):
      raise PlanUpdateValidationError(
        "Plan update payload must be an object."
      )
    ...
    if action not in VALID_PLAN_UPDATE_ACTIONS:
      raise PlanUpdateValidationError(f"Unsupported plan update action: '{action}'.")
    ...
    return cls(
      action=typed_action,
      new_steps=cls._parse_step_specs(
        payload.get("steps")
      ),
    )
  ...
  def _optional_text(
    ...
    *,
    ...
    if not isinstance(value, str):
      raise PlanUpdateValidationError(f"Plan update field '{field_name}' must be a string or null.")
  ...
  def _required_text(
    ...
    *,
    ...
    if not isinstance(value, str):
      raise PlanUpdateValidationError(f"Plan update field '{field_name}' must be a non_empty string.")
  ...
  def _parse_step_specs(
    ...
    if not raw_steps:
      raise PlanUpdateValidationError("Plan update must append at least one step.")
    ...
    for index, raw_step in enumerate(raw_steps, start = 1):
      if not isinstance(raw_step, dict):
        raise PlanUpdateValidationError(f"Appended plan step {index} must be an object.")
      
      unknow_fields = set(raw_step) - allowed_fields

      if unknow_fields:
        raise PlanUpdateValidationError(f"Append plan step {index} contains unsupported fiels: " + ",".join(sorted(unknow_fields)))
      
      description = raw_step.get("description")
      completion_criteria = raw_step.get("completion_criteria")

      if not isinstance(description, str):
        raise PlanUpdateValidationError(f"Appended plan step {index} feild 'description' must be a string.")
      
      if completion_criteria is not None and not isinstance(completion_criteria, str):
        raise PlanUpdateValidationError(f"Appended plan step {index} field 'completion_criteria' must be a string or null.")
      ...
      try:
        ...
        raise PlanUpdateValidationError(f"Appended plan step {index} is invalid: {error}") from error
  ...
  def apply_update(
    ...
    try:
      if update.action == "complete_current_step":
        plan.complete_current_step(result=update.result,)

      elif update.action == "skip_current_step":
        plan.skip_current_step(reason=update.reason,)

      elif update.action == "fail_current_step":
        if update.error is None:
          raise PlanUpdateValidationError("fail_current_step requires an error.")

        plan.fail_current_step(error=update.error,)

      elif update.action == "append_steps":
        if not update.new_steps:
          raise PlanUpdateValidationError("append_steps requires at least one step.")

        if len(update.new_steps)> self.policy.max_added_steps_per_update:
          raise PlanUpdateBudgetError(f"A single update may add at most {self.policy.max_added_steps_per_update} steps.")

        new_total = (len(plan.steps) + len(update.new_steps))

        if new_total > self.policy.max_total_steps:
          raise PlanUpdateBudgetError(f"The plan may contain at most {self.policy.max_total_steps} steps.")
        ...
        raise PlanUpdateValidationError(f"Unsupported plan update action: '{update.action}'.")
    except PlanUpdateError:
      raise
    except PlanError as error:
      raise PlanUpdateApplicationError(f"Plan update could not be applied: {error}") from error
