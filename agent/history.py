...
class ToolHistory:
  ...
  def count(self, tool_name: str, arguments: dict[str, Any]) -> int:
    return sum(1 for record in self.records if (record.tool_name == tool_name and record.arguments == arguments))

  def repeated(self, tool_name: str, arguments: dict[str, Any], threshold: int = 2) -> bool:
